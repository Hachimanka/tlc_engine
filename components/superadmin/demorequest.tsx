...
const demoRequests = [
	{
		...
		plan: null,
		status: { label: "New", color: "text-blue-600" },
		date: "Mar 20, 2026",
	},
	{
	...
	},
	{
		...
		organization: "Pacific Technical College",
	...
	return (
		<div className="w-full px-8 py-6">
			<div className="flex items-center mb-4">
				<h1 className="text-2xl font-bold text-teal-800 flex-1">DEMO REQUESTS</h1>
				...
				<table className="min-w-full bg-white rounded-xl shadow text-sm overflow-hidden">
					...
					<tbody>
						{filtered.map((req, idx) => (
							<tr key={req.name + req.email} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
								<td className="px-4 py-2 whitespace-nowrap">{req.name}</td>
								<td className="px-4 py-2 whitespace-nowrap">{req.organization}</td>
								<td className="px-4 py-2 whitespace-nowrap">
									{req.email.includes("@") ? (
										<a href={`mailto:${req.email}`} className="text-blue-600 underline">{req.email}</a>
									) : req.plan ? (
										<span className={req.plan.color}>{req.plan.label}</span>
									) : null}
								</td>
								<td className={`px-4 py-2 whitespace-nowrap font-medium ${req.status.color}`}>{req.status.label}</td>
